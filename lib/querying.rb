...
def select_series_title_with_most_human_characters
  "SELECT series.title FROM characters INNER JOIN series ON characters.series_id = series.id WHERE characters.species = 'human' GROUP BY characters.series_id ORDER BY COUNT(characters.species) DESC LIMIT 1"
end
